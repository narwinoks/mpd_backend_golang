...
func ModuleMiddleware(repo module.ModuleRepository, requiredModuleCode string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if found := cacher.Get(cacheKey, &moduleCodes); !found {
			modules, err := repo.GetUserModules(userID, roleID)
			if err != nil {
				c.Error(err)
				c.Abort()
				return
